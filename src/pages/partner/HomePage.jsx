...
import { InventoryChart, OrderChart, ProductChart } from '../../component/partner/dashboard/DashboardChart';
...
export default function HomePage() {
  ...
  const updateProductData = (inventories) => {
    const labels = inventories.flatMap(inventory => inventory.products.map(p => p.name));
    const data = inventories.flatMap(inventory => inventory.products.map(p => p.quantity));
  ...
  return (
    <div className="p-6">
      ...
      {/* Main Flex Layout: Chart on the Left, Overview on the Right */}
      <div className="flex space-x-6 mt-6">
        {/* Left: Chart with Tab/Slider */}
        <div className="w-1/2">
          <SliderTabs activeChart={activeChart} setActiveChart={setActiveChart} />
          {renderChart()}
        </div>
